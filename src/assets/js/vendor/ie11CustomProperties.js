...
!(function () {
  ...
  function onElement (selector, callback) {
    ...
    var els = qsa(root, listener.selector); var i = 0; var el
    while (el = els[i++]) {
  ...
  function checkListeners (inside) {
    var i = 0; var listener
    while (listener = listeners[i++]) checkListener(listener, inside)
  }
  function checkMutations (mutations) {
    var j = 0; var i; var mutation; var nodes; var target
  ...
  if (!('sheet' in SVGStyleElement.prototype)) {
    Object.defineProperty(SVGStyleElement.prototype, 'sheet', {
      get: function () {
        var all = document.styleSheets; var i = 0; var sheet
        while (sheet = all[i++]) {
  ...
  function parseRewrittenStyle (style) { // less memory then parameter cssText?
    ...
    var matchesGetters = cssText.match(regRuleIEGetters); var j; var match
    if (matchesGetters) {
  ...
  function activateStyleElement (style, css) {
    ...
    var rules = style.sheet.rules; var i = 0; var rule // cssRules = CSSRuleList, rules = MSCSSRuleList
    while (rule = rules[i++]) {
  ...
  function redrawStyleSheets () {
    for (var prop in styles_of_getter_properties) {
      const styles = styles_of_getter_properties[prop]
  ...
  function selectorAddPseudoListeners (selector) {
    ...
    for (var pseudo in pseudos) {
      ...
      if (parts.length > 1) {
        ...
        const sel = unPseudo(parts[0] + ending)
  ...
  function _drawElement (el) {
    ...
    for (var prop in el.ieCPSelectors) {
      var important = style['-ieVar-❗' + prop]
      const valueWithVar = important || style['-ieVar-' + prop]
      if (!valueWithVar) continue // todo, what if '0'
      var details = {}
      var value = styleComputeValueWidthVars(style, valueWithVar, details)
  ...
  function findVars (str, cb) { // css value parser
    let level = 0; let openedLevel = null; let lastPoint = 0; let newStr = ''; let i = 0; let char; let insideCalc
    while (char = str[i++]) {
      if (char === '(') {
        ++level
      ...
      if (char === ')' && openedLevel === level) {
        let variable = str.substring(lastPoint, i - 1).trim(); let fallback
        const x = variable.indexOf(',')
        if (x !== -1) {
